...
agentd_count=$(ps -ef | grep agentd | grep -v grep | grep -v gdb | wc -l)
...
PING_SENTINEL_PAYLOAD_SIZE=5000000 ./ping_sentinel &

echo "Sleeping to let ping sentinel start."
sleep 2

#run the ping client
PING_CLIENT_PAYLOAD_SIZE=5000000 ./multi_ping_client
